...
export default function InventoryAnalytics() {
  ...
  const calculateKPIs = (vehicles: any[], transactions: any[]) => {
    ...
    const avgDaysInStock = liveVehicles.length > 0
              ? liveVehicles.reduce((sum, v) => sum + differenceInDays(now, new Date(v.created_at)), 0) / liveVehicles.length
      : 0;
  ...
  const generateChartData = (vehicles: any[], transactions: any[]) => {
    ...
    const agingHeatmap = VEHICLE_CLASSES.slice(1).map(vClass => {
      ...
      return {
        ...
        data: [
                  { value: classVehicles.filter(v => differenceInDays(now, new Date(v.created_at)) <= 15).length },
        { value: classVehicles.filter(v => differenceInDays(now, new Date(v.created_at)) > 15 && differenceInDays(now, new Date(v.created_at)) <= 30).length },
        { value: classVehicles.filter(v => differenceInDays(now, new Date(v.created_at)) > 30 && differenceInDays(now, new Date(v.created_at)) <= 60).length },
        { value: classVehicles.filter(v => differenceInDays(now, new Date(v.created_at)) > 60 && differenceInDays(now, new Date(v.created_at)) <= 90).length },
        { value: classVehicles.filter(v => differenceInDays(now, new Date(v.created_at)) > 90).length }
        ]
      };
    });
