...
// Temporary shim to unblock build; replace with real service when ready
const documentUploadService = {
  async getVehicleDocuments(vehicleId: string): Promise<any[]> { return []; },
  async uploadVehicleDocument(vehicleId: string, file: File, type: string, onProgress?: (p:number)=>void): Promise<{success: boolean; file_url?: string; file_name?: string; ocrData?: any; error?: string;}> { onProgress && onProgress(100); return { success: true, file_url: URL.createObjectURL(file), file_name: file.name, ocrData: {} }; },
  async deleteVehicleDocument(documentId: string): Promise<boolean> { return true; }
};
type VehicleDocument = { id: string; document_type: string; file_url: string; created_at?: string };
import { useToast } from '@/components/ui/use-toast';
// OCR and activation hooks/components removed for now; wire back when available
const DocumentOCR: React.FC<any> = () => null;
const useDealerActivationSettings = () => ({ checkFeatureAccess: (_key?: string) => false });
