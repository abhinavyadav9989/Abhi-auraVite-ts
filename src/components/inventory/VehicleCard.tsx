...
import { Car, IndianRupee, Share2, MoreVertical, Calendar, Gauge } from "lucide-react";
...
export default function VehicleCard({ vehicle, isSelected, onSelect, onShare }: VehicleCardProps) {
  ...
  return (
    <Card className="overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 relative group">
      <CardHeader className="absolute top-2 left-2 z-10 p-0">
        <Checkbox
          checked={isSelected}
          onCheckedChange={(checked) => onSelect(vehicle.id, checked)}
